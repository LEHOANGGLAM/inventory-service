...
import com.yes4all.service.dto.AdminUserDTO;
import com.yes4all.service.dto.UserDTO;
import com.yes4all.service.dto.request.UserLoginDTO;
import com.yes4all.service.dto.response.UserLoginResponseDTO;
...
public class UserService {
    ...
    @Value("${spring.security.oauth2.client.registration.oidc.code}")
    ...
    public Object doAuthorize(UserLoginDTO request) {
        try {
            ...
            String formData = getFormDataString(formParams);
            ...
            if (responseCode == HttpStatus.OK.value()) {
                ...
                return objectMapper.readValue(responseContent.toString(), UserLoginResponseDTO.class);
            } else {
                throw new BusinessException(String.format("Cannot authorizes credential. Response code: %d", responseCode));
            }
        } catch (IOException ex) {
            log.error("Fail to get access token. Message = {}", ex.getMessage());
            throw new BusinessException("Fail to get access token.");
        }
    }

    private String getFormDataString(Map<String, String> parameters) {
